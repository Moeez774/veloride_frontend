...
const VerifyPax = ({ passengers, ride, setSelectedRide, setActiveRides, activeRides }: { passengers: any[], ride: any, setSelectedRide: any, setActiveRides: any, activeRides: any[] }) => {
  ...
  const dropOffPassenger = async (passenger: any) => {
    const updatedRide = await markPassengerDroppedOff(ride._id, passenger.userId)
              ...
              <div className={`relative ${isChoosed ? '-translate-x-full' : 'translate-x-0'} w-full transition-all duration-300`}>
                    ...
                    return (
                      <div key={index}>
                        <div className='flex items-center justify-between gap-2'>
                          ...
                          {e.status === 'verified' && <button className='p-2.5 hover:bg-[#00563ccc] cursor-pointer transition-all duration-200 rounded-full bg-[#00563c] text-white' onClick={() => dropOffPassenger(e)}><CheckCheck size={18} color='white' /></button>}
                        </div>
                ...
                <div className='absolute w-full flex flex-col gap-12 px-6 translate-x-full h-full top-0 left-0'>
                  ...
                  <div className='flex w-full justify-center items-center flex-col gap-2'>
                    <button className='bg-[#00563c] text-white cursor-pointer hover:bg-[#00563ccc] px-6 py-3 rounded-md max-w-[300px] mx-auto w-full mt-4' onClick={verifyPassenger}>Verify</button>
