...
const SearchRides = ({ toggleTheme, activeRides, setActiveRides }: { toggleTheme: boolean, activeRides: any[], setActiveRides: any }) => {
    ...
    return (
        <div className={`mt-4 flex px-3 items-center gap-2 w-full ${toggleTheme ? 'bg-[#000000] text-[#b1b1b1]' : 'bg-[#fefefe] text-[#5b5b5b]'}`}>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent className={`${toggleTheme ? 'bg-[#202020] border-[#353535] border text-[#fefefe]' : 'border bg-[#fefefe] text-[#202020]'}`}>
                    <DropdownMenuItem onClick={() => setPlaceHolder('by ID')}>By ID</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setPlaceHolder('by Status')}>By Status</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setPlaceHolder('by Date')}>By Date</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setPlaceHolder('by Time')}>By Time</DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
    )
