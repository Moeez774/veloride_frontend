  ...
  return (
    <div className={`h-[97vh] pt-6 w-full gap-4 flex flex-col ${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'}`}>
      ...
      <div className='flex flex-col sm:flex-row gap-4 sm:gap-0 sm:justify-between sm:items-center'>
        {user && <h1 className='flex items-center ml-2 md:ml-0 gap-1 text-2xl'><Menu onClick={() => setShowSideBar(true)} size={23} className='mr-2 cursor-pointer md:hidden' color={toggleTheme ? '#fefefe' : '#202020'} /> Welcome back, <p className='font-semibold'>{user?.fullname ? user?.fullname.split(" ")[0] : ""}!</p></h1>}

      { activePage==='wallet' && <div className='mr-1 md:mr-6 flex items-center gap-2'>
          ...
          <button className='flex py-2.5 px-[0.90rem] bg-[#00563c] rounded-md text-[#fefefe] items-center gap-1 text-sm md:text-base'><DownloadCloud size={20} /> Export</button>
        </div> }
      </div>
