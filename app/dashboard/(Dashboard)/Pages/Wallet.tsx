...
import { Calendar, ChevronDown, ArrowUp, Copy, Plus, CheckCircle, Clock } from 'lucide-react'
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
            ...
            {data.length != 0 && data.map((transaction, index) => (
                <React.Fragment key={index}>
                    <div className={`grid grid-cols-3 mt-3 text-[15px] ${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'} font-medium gap-x-4`}>
                        <h1>{transaction.rideId}</h1>
                        <h1>{transaction.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</h1>
                        <h1 className={transaction.type === "Send" ? 'text-[#FF8080]' : 'text-[#048c64]'}>
                            {transaction.type === "Send" ? '-' + transaction.amount : '+' + transaction.amount}
                        </h1>
                    </div>
                    {index !== 6 && <hr className={`${toggleTheme ? 'border-[#202020]' : 'border-[#f0f0f0]'} mt-3`} />}
                </React.Fragment>
            ))}
...
const Wallet = ({ toggleTheme, user, formattedDate }: WalletProps) => {
    ...
    return (
        <div className={`${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'} w-full md:pr-4 h-full overflow-y-auto pb-3`} style={{ scrollbarWidth: 'thin' }}>

            <div className='flex max-w-7xl mx-auto w-full flex-col gap-y-4'>
                <div className='font-medium gap-x-6 flex flex-col lg:flex-row w-full'>
                    ...
                    <div className='flex flex-col lg:w-3/4 xl:w-1/2 gap-y-5'>
                        <div className={`w-full p-5 ${toggleTheme ? 'bg-[#0d0d0d] border border-[#202020]' : 'bg-[#fefefe] border'} rounded-xl  h-auto`}>
                            ...
                            <h1 className='text-4xl mt-4 flex items-center gap-4'>PKR {walletData.balance.currentBalance}
                                <span className={`${toggleTheme ? 'text-[#d2f5d9] bg-[#01b580]' : 'text-[#01b580] bg-[#D2F5D9]'} flex items-center py-1 px-1.5 rounded-full text-xs`}><ArrowUp size={14} /> {walletData.balance.percentageChange}%</span>
                            </h1>
                        ...
                        <div className={`w-full mb-4 lg:mb-0 h-[28.5em] ${toggleTheme ? 'bg-[#0d0d0d] border border-[#202020]' : 'bg-[#fefefe] border'} rounded-xl p-5`}>
                ...
                <div className='flex flex-col lg:flex-row items-center gap-4'>
                    <div className={`w-full h-auto ${toggleTheme ? 'bg-[#0d0d0d] border border-[#202020]' : 'bg-[#fefefe] border'} rounded-xl p-5`}>
                        ...
                        <h1 className={`text-[13px] ${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#9F9F9F]'} mt-5 flex items-center gap-[0.90rem]`}>
                            <span className={`${toggleTheme ? 'text-[#d2f5d9] bg-[#01b580]' : 'text-[#01B580] bg-[#D2F5D9]'} flex items-center py-1 px-1.5 rounded-full text-xs`}><ArrowUp size={14} /> {walletData.monthlySpent[walletData.monthlySpent.length - 1].percentageChange}%</span> Compared to last month
                        </h1>
                    ...
                    <div className={`${toggleTheme ? 'bg-[#0d0d0d] border border-[#202020]' : 'bg-[#fefefe] border'} rounded-xl p-5 w-full`}>
                        ...
                        <h1 className={`text-[13px] ${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#9F9F9F]'} mt-5 flex items-center gap-[0.90rem]`}>
                            <span className={`${toggleTheme ? 'text-[#ffdede] bg-[#ff8080]' : 'text-[#FF8080] bg-[#FFDEDE]'} flex items-center py-1 px-1.5 rounded-full text-xs`}><ArrowUp className='rotate-180' size={14} /> {walletData.monthlyEarnings[walletData.monthlyEarnings.length - 1].percentageChange}%</span> Compared to last month
                        </h1>
                ...
                <div className={`w-full h-auto ${toggleTheme ? 'bg-[#0d0d0d] border border-[#202020]' : 'bg-[#fefefe] border'} rounded-xl pt-5 px-5`}>
                    ...
                    <div className='flex items-center justify-between'>
                        <h1 className={`mt-3 text-sm ${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'}`}>Total Balance
                            <p className={`text-2xl mt-2 ${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'} font-medium`}>PKR {walletData.totalEarned}</p>
                        </h1>
                ...
                <div className={`w-full h-[30em] ${toggleTheme ? 'bg-[#0d0d0d] text-[#fefefe] border border-[#202020]' : 'bg-[#fefefe] border text-[#202020]'} rounded-xl pb-[0.90rem] pt-5 px-5`}>
                    <label className='text-xl inter font-medium'>Pending Transactions</label>

                    <Table className='w-full mt-3 h-full'>
                        <TableCaption className={`text-sm mt-6 inter ${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'} mb-auto`}>{walletData.pendingTransactions.length != 0 ? "A list of your recent invoices." : "No pending transactions."}</TableCaption>
                        <TableHeader>
                            <TableRow className={`${toggleTheme ? 'text-[#b1b1b1] hover:bg-[#202020]' : 'text-[#5b5b5b] hover:bg-[#f0f0f0]'} text-sm inter`}>
                                <TableHead className={`${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'} w-[150px]`}>Ride ID</TableHead>
                                <TableHead className={`${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'} w-[150px] sm:w-auto`}>Type</TableHead>
                                <TableHead className={`${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'} w-[300px] sm:w-auto`}>Recipient</TableHead>
                                <TableHead className={`${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'} w-[150px] sm:w-auto text-right`}>Status</TableHead>
                                <TableHead className={`${toggleTheme ? 'text-[#b1b1b1]' : 'text-[#5b5b5b]'} w-[150px] sm:w-auto text-right`}>Amount</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody className='h-full'>
                            {walletData.pendingTransactions.length != 0 && walletData.pendingTransactions.map((transaction, index) => (
