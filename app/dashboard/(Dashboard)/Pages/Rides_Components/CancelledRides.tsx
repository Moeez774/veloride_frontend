...
import { MapPinIcon, Navigation, Users2, CreditCard, EllipsisVertical, Trash, Users } from 'lucide-react'
                    ...
                    {rides.map((ride, index) => (
                        <div key={index} className={`w-full py-4 rounded-xl ${toggleTheme ? 'border-[#202020] border' : 'border'} bg-transparent`}>
                            ...
                            <div className='flex items-center justify-between px-4'>
                                <h1 className='font-medium'>Cancelled on {formatDate(ride.rideDetails.date)}</h1>
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent className={`${toggleTheme ? 'bg-[#202020] text-[#fefefe]' : 'bg-[#fefefe] text-[#202020]'} border-none`}>
                                        ...
                                        <DropdownMenuItem className={`cursor-pointer ${toggleTheme ? 'text-[#fefefe] hover:bg-[#353535]' : 'text-[#202020] hover:bg-[#f0f0f0]'} flex items-center gap-1`}><Trash size={18} /> Delete Ride</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
