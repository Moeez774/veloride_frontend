        ...
        <div className={`inter w-[80vw] sm:w-[30rem] lg:w-[22rem] flex flex-col gap-5 ${toggleTheme ? 'text-[#fefefe]' : 'text-[#202020]'}`}>
            ...
            <div className='flex flex-col gap-3'>
                ...
                <div className='flex flex-col gap-1.5'>
                    <label htmlFor="" className={`inter font-medium text-[12px] ${toggleTheme ? 'text-[#b1b1b1]' : ''}`}>Pickup Location</label>

                    <div className={`flex items-center ${toggleTheme ? 'bg-[#1f1f1f]' : 'bg-[#EAEAEA]'} justify-between w-[80vw] sm:w-[30rem] lg:w-[22rem] rounded-md border-solid px-3 gap-1 shadow-md`}>
                        <input type='text' ref={inputRef} value={pickup ? pickup : ''} onChange={async (e) => await (handleInputChange(e, setPickup, setSuggestions1))}
                            placeholder='Pickup Location'
                            className={`py-3 bg-transparent ${toggleTheme ? 'placeholder:text-[#717171] text-[#fefefe]' : 'placeholder:text-[#a4a4a4] text-[#202020]'} text-[14px] lg:text-base flex-1 outline-none inter`}
                            ...
                            <TooltipProvider>
                                <Tooltip open={showPickupTip} onOpenChange={() => setShowPickupTip(!showPickupTip)}>
                                    <TooltipTrigger className='translate-y-[4px]'>
                                        <HelpCircle onClick={() => setShowPickupTip(true)} onFocus={() => setShowPickupTip(true)}
                                            onBlur={() => setShowPickupTip(false)} size={20}
                                            color={toggleTheme ? '#fefefe' : '#202020'}
                                        />
                                    </TooltipTrigger>
                                    <TooltipContent className={`relative z-[200] bg-[#00563c]`}>
                ...
                <div className='flex flex-col gap-1.5'>
                    <label htmlFor="" className={`inter font-medium text-[12px] ${toggleTheme ? 'text-[#b1b1b1]' : ''}`}>Drop-off Location</label>
                    <div className={`flex items-center ${toggleTheme ? 'bg-[#1f1f1f]' : 'bg-[#EAEAEA]'} justify-between w-[80vw] sm:w-[30rem] lg:w-[22rem] rounded-md border-solid px-3 gap-1 shadow-md`}>
                        <input type='text' value={drop ? drop : ''} ref={inputRef2}
                        ...
                        <div>
                            <TooltipProvider>
                                <Tooltip open={showDropoffTip} onOpenChange={() => setShowDropoffTip(!showDropoffTip)}>
                                    <TooltipTrigger className='translate-y-[4px]'>
                                        <HelpCircle onClick={() => setShowDropoffTip(true)} onFocus={() => setShowDropoffTip(true)}
                                            onBlur={() => setShowDropoffTip(false)} size={20}
                                            color={toggleTheme ? '#fefefe' : '#202020'}
                                        />
                                    </TooltipTrigger>
                                    <TooltipContent className={`relative z-[200] bg-[#00563c]`}>
            ...
            <div className='w-full flex flex-col gap-6'>
                ...
                <div className='w-full flex items-center justify-between'>
                    ...
                    <div className='w-[6.5rem] flex items-center gap-2 sm:gap-4 justify-between'>
                        <h1 className={`text-[14px] sm:text-sm font-normal ${toggleTheme ? 'text-[#fefefe]' : ''}`} style={{ userSelect: 'none' }}>Passengers</h1>

                        <div className='flex gap-2 items-center'>
                            <h1 className={`text-[14px] sm:text-sm w-4 ${toggleTheme ? 'text-[#fefefe]' : ''}`}>{passengers < 10 ? `0${passengers}` : passengers}</h1>

                            <div className='flex flex-col gap-1'>

                                <div onClick={() => {
                                    if (passengers < 4) setPassengers(passengers + 1)
                                }} className={`p-1 cursor-pointer ${toggleTheme ? 'bg-[#1f1f1f]' : 'bg-[#eaeaea]'} rounded-full shadow-md`}>
