                    ...
                    <div className='flex flex-col justify-center my-4 items-center gap-5'>
                        ...
                        <div className='relative w-full justify-center flex items-center gap-1'>
                            ...
                            <div className='absolute w-full items-end flex flex-col'>
                                <div className='px-1.5 hover:bg-[#f0f0f0] cursor-pointer transition-all duration-200 rounded-sm py-0.5' onClick={() => {
                                    if (percentOff < ((avgPrice / 10) + (avgPrice / 10))) {
                                        setBudget(prev => prev + 1)
                                        setPercentOff(prev => prev + 1)
                                    }
                                }}><ChevronUp size={22} className='text-[#00b37e]' /></div>
                                <div className='px-1.5 py-0.5 hover:bg-[#f0f0f0] cursor-pointer transition-all duration-200 rounded-sm' onClick={() => {
                                    if (percentOff > 0) {
                                        setBudget(prev => prev - 1)
                                        setPercentOff(prev => prev - 1)
                                    }
                                }}><ChevronDown size={22} color='#00b37e' /></div>
                            </div>
                        </div>
                    </div>
                </div>
