...
const Overview = ({ ride, isBest }) => {
    ...
    const data = [
        {
            ...
            detail: <h1 className='flex items-center my-1 text-xs'><Dot size={25} color='#202020' />{ride.preferences.rideType}</h1>,
        },
        {
        ...
        {
            head: <h1 className='exo2 font-semibold flex items-center gap-1 text-lg'><Accessibility size={22} color='#202020' />Accessability needs</h1>,
            detail: <h1 className='flex items-center text-xs'><Dot size={25} color='#202020' />{ride.preferences.needs.wheelchairAccess ? 'Wheelchair available' : 'No accessability resource available'}</h1>
        },
        {
    ...
    return (
        <div className='px-2 sm:px-6 flex flex-col gap-2'>
            {data.map((e, index) => {
                return (
                    <div key={index} className={`bg-[#f0f0f0] cursor-pointer transition-all duration-200 overflow-hidden ${state[index] && (index != data.length - 1 && index != 1) ? 'h-[6.1rem]' : state[index] && index === 1 ? 'h-[7.4rem] sm:h-[6.1rem]' : state[index] && index === data.length - 1 ? 'h-[11.5rem]' : 'h-[3.7rem]'} flex flex-col gap-3 p-4`} onClick={() => setState(prevState => prevState.map((item, i) => (i === index ? !item : false)))}>
