    ...
    return (
        <div className='flex items-center'>
            <Sheet open={openChat} onOpenChange={() => setOpenChat(false)}>
                ...
                <SheetContent style={{ scrollbarWidth: 'thin' }} className='w-screen sm:max-w-lg px-0 mx-0 py-1 min-h-screen sm:w-full'>
                        ...
                        <div className='inter w-full relative flex flex-col gap-0'>
                            ...
                            <div className='flex justify-between pt-6 px-4 items-center'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <div>
                                        <h1 className='inter text-[17px] flex items-center gap-1'>{pathname.startsWith('/ride-detail') ? 'Driver' : receiver.fullname}<span className='text-[12px] font-normal translate-y-0.5' style={{ display: pathname.startsWith('/ride-detail') ? 'block' : 'none' }}>({receiver.fullname})</span></h1>
                                        <h1 className='flex font-medium items-center gap-1 text-xs'><StarIcon color='#202020' className='w-4 h-4' />4.5</h1>
                                    </div>
                                </div>
                                <MoreVertical size={20} color='#202020' />
                            ...
                            <div className='pb-16 flex flex-col gap-8'>
                                    ...
                                    {chat.length!=0 && chat.map((e, index) => {
                                        return (
                                            <div key={index} className={`flex ${e.sender_id === user?._id ? 'items-end' : 'items-start'} flex-col gap-1.5`}>
                                                <div className={`${e.sender_id === user?._id ? 'bg-[#A5E8D4]' : 'bg-[#f0f0f0]'} rounded-xs py-4 w-fit max-w-3/5 lg:max-w-sm pl-2.5 pr-10`} style={{ wordBreak: 'break-word' }}>
                                ...
                                {/* inputs */}
                                <div className='fixed w-screen sm:max-w-lg sm:w-full bottom-0 px-6 bg-[#f0f0f0] h-16 rounded-tr-3xl rounded-tl-3xl flex items-center justify-between gap-3'>
                                    <input type="text" onKeyDown={(e) => {
                                        if(e.key==="Enter") messageIt()
                                    }} value={message} onChange={(e) => setMessage(e.target.value)} className='flex-1 py-2.5 text-[15px] md:text-base text-[#202020] outline-none' placeholder='Type a message...' />

                                    <div onClick={() => messageIt()} className='p-2 active:bg-[#00b36e] transition-all duration-200 hover:bg-[#00b37daf] cursor-pointer rounded-full bg-[#00b37e]'>
                                        <LucideForward size={25} color='#f0f0f0' />
                                    </div>
                                </div>
                            </div>
                        </div>
